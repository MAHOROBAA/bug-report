    ...
    <div class="filter_wrapper">
      <div class="filter_row">
        ...
        <div class="form_field">
          ...
          <input id="searchDate" type="text" v-model="searchDate" placeholder="YYYYMMDD" maxlength="8" @input="filterDateFormat" />
        </div>
      </div>
      <div class="filter_row">
        <!-- 검색어 -->
        <div class="form_field">
          ...
          <input id="searchKeyword" type="text" v-model="searchKeyword" placeholder="검색어를 입력하세요" />
        </div>
        <!-- 검색 버튼 -->
        <button class="search_btn" :disabled="!isSearchEnabled" @click="handleSearch">검색</button>
      </div>
    </div>
    <div class="line"></div>
    <!-- 빈 상태 -->
        ...
        <div class="item_inner">
          <div class="item_left">
            <div class="item_icon" :class="{ rotating: report.isRotating }" @click="toggleExpand(report.id)">
              ...
              <img :key="report.isOpen" :src="report.isOpen ? closeIcon : openIcon" alt="toggle icon" class="icon_img" />
              </transition>
            </div>
          </div>
            ...
            <div class="item_info">
              <div class="item_row">
                <div class="item_field">
                  ...
                  <CustomSelect v-else v-model="report.category" class="edit_select" />
                </div>
                <div class="item_field">
                  <label>발생시각</label>
                  <p v-if="!report.isEditing" class="item_date">{{ report.occurredAt }}</p>
                  <input v-else type="text" v-model="report.occurredAt" placeholder="YYYY-MM-DD HH:MM" maxlength="16" />
                </div>
              </div>
              <p class="item_registered web"> 등록일자 {{ formatDate(report.createdAt) }} </p>
            </div>
            <!-- 펼침 영역 -->
            <div v-if="report.isOpen" class="item_detail">
              <div class="item_field">
                ...
                <div class="item_contents">
                  ...
                  <textarea v-else v-model="report.content" class="edit_textarea" rows="3" />
                </div>
              </div>
              <div class="item_buttons">
                <button class="btn_edit" @click="report.isEditing ? saveReport(index) : editReport(index)">{{ report.isEditing ? '저장' : '수정' }}</button>
